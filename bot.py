...
def calculate_fees_and_profit(supplier_price, ebay_price, shipping_cost=0.0):
    ebay_fee = (ebay_price * 0.1325) + 0.30
    payout_fee = ebay_price * 0.015
    total_fees = ebay_fee + payout_fee
    net_profit = ebay_price - supplier_price - shipping_cost - total_fees
    margin = (net_profit / ebay_price) * 100 if ebay_price > 0 else 0
    return total_fees, net_profit, margin
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = (
        ...
        "🏷 `/title <məhsul adı>` - Hazır şablonlarla SEO başlıqlar təklif edir.\n\n"
        "_Misallar:_\n"
        "• `/profit 10 25`\n"
...
async def trend_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, item in enumerate(selected, 1):
        fees, profit, margin = calculate_fees_and_profit(item["supplier_price"], item["est_ebay_price"])
        response += (
            f"**{i}. {item['title']}**\n"
...
async def profit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        res = (
            ...
            f"💸 **Tahmini eBay & Çəkim Komissiyası:** `${fees:.2f}`\n"
            f"───────────────\n"
...
async def title_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if not args:
        await update.message.reply_text("⚠️ **Format:** `/title <məhsul_adı>`\n\n_Misal:_ `/title sunset lamp`", parse_mode="Markdown")
        return
    ...
    q, f, s, v = random.choice(qualities), random.choice(features), random.choice(shipping_triggers), random.choice(value_props)
    t1 = f"{q} {raw_input} {f} - {s}"[:80]
    t2 = f"{raw_input} {f} {v} - {s}"[:80]
    
    res = (
        f"🎯 **`{raw_input}` üçün Unikal eBay SEO Başlıqları:**\n\n"
        f"1️⃣ `{t1}`\n"
        f"2️⃣ `{t2}`\n\n"
        f"💡 _Üstünə basaraq kopyalaya bilərsən._"
    )
    await update.message.reply_text(res, parse_mode="Markdown")
